 ...
 * @access public
 */
function getReadableTime(ms, options = {}) {
    if (!ms || typeof ms !== 'number' || !isFinite(ms))
        return '0s';
    const t = getTimeObject(ms);
    const reply = [];
    if (!options.showMS && ms <= 1000)
        throw new TypeError(`Final value is smaller than 1 second (Exactly: ${ms}ms). To show milliseconds use the showMS option.`);
    if (options.compact) {
        if (options.showEmpty ? true : t.years)
            reply.push(`${t.years}y`);
        if (options.showEmpty ? true : t.months)
            reply.push(`${t.months}mo`);
        if (options.showEmpty ? true : t.weeks)
            reply.push(`${t.weeks}w`);
        if (options.showEmpty ? true : t.days)
            reply.push(`${t.days}d`);
        if (options.showEmpty ? true : t.hours)
            reply.push(`${t.hours}h`);
        if (options.showEmpty ? true : t.minutes)
            reply.push(`${t.minutes}m`);
        if (options.showEmpty ? true : t.seconds)
            reply.push(`${t.seconds}s`);
        if (options.showMS ? options.showEmpty ? true : t.milliseconds : false)
    ...
    if (reply.length > 0)
        ...
        throw new TypeError(`There is nothing to display.${options.showMS && ms > 0 ? ' To show milliseconds use the showMS option.' : ''}`);
}
